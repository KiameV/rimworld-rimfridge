...
namespace RimFridge
{
    internal class CompFrosty : ThingComp
    {
        ...
        public override void PostSplitOff(Thing piece)
        {
            ...
            bool flag = thingWithComps.TryGetComp<CompFrosty>() == null;
            if (flag)
            {
        ...
        public override void CompTickRare()
        {
            ...
            if (flag)
            {
                num = this.parent.PositionHeld.GetTemperature(this.parent.MapHeld);
            }
            ...
            if (flag2)
            {
                ...
                if (flag3)
                {
                    num = casterPawn.PositionHeld.GetTemperature(casterPawn.MapHeld);
                }
            }
            bool spawned = this.parent.Spawned;
            if (spawned)
            {
                List<Thing> thingList = this.parent.PositionHeld.GetThingList(this.parent.MapHeld);
